...
public class CharacterMovement : MonoBehaviour
{
    ...
    protected virtual void HandleMovement(float x, float y)
    {
        ...
        movementHit = Physics2D.BoxCast(transform.position, myCollider.size, 0f, new Vector2(0f, moveDelta.y), Mathf.Abs(moveDelta.y * Time.deltaTime), LayerMask.GetMask(TagManager.BLOCKING_LAYER_MASK));
        ...
        movementHit = Physics2D.BoxCast(transform.position, myCollider.size, 0f, new Vector2(moveDelta.x, 0f), Mathf.Abs(moveDelta.x * Time.deltaTime), LayerMask.GetMask(TagManager.BLOCKING_LAYER_MASK));
